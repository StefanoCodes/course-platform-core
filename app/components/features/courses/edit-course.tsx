...
export function EditCourse({name,description,courseId,slug}:{
    name:string
    description:string
    courseId:string
    slug:string
            ...
            <DialogContent className="flex flex-col gap-8">
                ...
                <Form {...form}>
                    <fetcher.Form method="POST" action="/resource/course" className="flex flex-col gap-4" onSubmit={form.handleSubmit((data) => {
                        fetcher.submit({ ...data, intent: "edit-course", id: courseId, slug: slug }, {
                        ...
                        {/* Submit button */}
                        <Button type="submit" className="bg-brand-primary text-white cursor-pointer hover:bg-brand-primary/60 hover:text-white" disabled={isSubmitting}>{isSubmitting ? "Editing Course..." : "Edit Course"}</Button>
                    </fetcher.Form>
                </Form>
            </DialogContent>
        </Dialog>
