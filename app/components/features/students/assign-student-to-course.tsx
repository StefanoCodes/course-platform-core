...
export function AssignStudentToCourse({form}: {form: UseFormReturn<CreateStudentSchema>}) {
  ...
  const handleUnselect = React.useCallback((course: Course) => {
    ...
    form.setValue("courses", form.getValues("courses").filter((s) => s !== course.id));
  }, []);
  ...
  const selectables = courses.filter(
    (course) => !selected.includes(course),
  );
  return (
    <div>
                  ...
                  return (
                    <CommandItem
                      ...
                      onSelect={(value) => {
                        ...
                        form.setValue("courses", [...form.getValues("courses"), course.id]);
